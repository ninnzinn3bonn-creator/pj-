...
**[チーム台帳を開く](https://project-manager-team.ninnzinn-3bonn.workers.dev)** · **[AI用の導入手順](docs/AI-SETUP.md)** · **[管理者の初回設定](team-worker/README.md)**
...
| 5 | GitHubでチームログインし、ローカル台帳から共有 | 他メンバーのチーム画面にも表示される |
...
チームメンバーがCloudflareの契約やGitHub Appの作成を行う必要はありません。管理者から共有データ用の非公開リポジトリへの招待を受け、本人のGitHubでログインします。クローンしただけではプラグイン導入・チーム接続は完了しません。

Claude Codeでは**プラグイン（スキル＋フック）**として導入します。Codex用とは別の形式を準備スクリプトが生成します。既存の進捗更新はローカル向けで、チームへの送信は共有ボタンまたは共有CLIから行います。

> 公開サイトの配置は完了しています。GitHubログインの初期設定が未完了の間は「設定中」と表示されます。管理者の承認完了後にチームログインを利用できます。
