...
| 5 | GitHubでチームログインし、ローカル台帳を接続 | 個人用と共有プロジェクトが同じ一覧に表示される |
...
チームメンバーがCloudflareの契約やGitHub Appの作成を行う必要はありません。管理者が接続ポータルでGitHubユーザー名をチームへ追加し、本人がGitHubでログインします。クローンしただけではプラグイン導入・チーム接続は完了しません。

Claude Codeでは**プラグイン（スキル＋フック）**として導入します。Codex用とは別の形式を準備スクリプトが生成します。既存の進捗更新はローカル向けで、チームへの送信は共有ボタンまたは共有CLIから行います。

> 接続ポータルのURLは公開されていますが、プロジェクト一覧は表示しません。共有データはGitHub認証とチームメンバー照合を通過したローカル台帳だけがD1から取得できます。
