...
1. 管理者が共有データ用の非公開リポジトリへ利用者をWrite権限で招待していることを確認する。招待の承認は本人に依頼する。
2. チームサイトで本人のGitHubアカウントにログインする。管理者用Client secretやSESSION_SECRETをメンバーへ配らない。
3. 「ローカル台帳と接続」で本人用の接続トークンを取得する。
4. ローカル台帳の「台帳ツール → チーム連携設定」へURLとトークンを入力し、接続確認を行う。トークンをチャット、Git、プロジェクトJSONに記録しない。
5. 利用者が指定したプロジェクトのみ「チームへ共有・更新」を実行する。概念図も送る場合は内容を確認する。
6. チームサイトで再読み込みし、同じIDのプロジェクトが表示されることを確認する。
...
「進捗に反映」はローカル台帳の更新。「概念図に反映」はローカル概念図の更新。共有は別途共有ボタン、または `team-worker/scripts/team-cli.mjs` を使う。現在は自動双方向同期ではない。
