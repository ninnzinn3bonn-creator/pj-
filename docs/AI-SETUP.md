...
1. チームサイトで本人のメールアドレスへ届くワンタイムコードを使ってログインする。初回利用者には本人専用チームが自動作成される。管理者用SESSION_SECRETをメンバーへ配らない。
2. 既存チームへ参加する場合だけ、そのチームの管理者がポータルから利用者のメールアドレスを追加する。個人利用や本人専用チームだけなら追加操作は不要。
3. 利用するチームを選び、「接続情報を表示」でURL・チームID・本人用の固定接続トークンを取得する。同じチームでは同じトークンが再表示され、本人が再発行するかメンバー解除されるまで有効である。
...
HTTP 503で「設定中」の場合は管理者のCloudflare Access設定待ち。401はログイン期限切れ、403はメンバー登録・Originを調べる。ポートが4170以外なら管理者が許可Originへ追加する必要がある。本人が許可していない接続先へ変更しない。
