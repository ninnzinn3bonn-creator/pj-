...
- `概念図に反映` / `概念図を反映`: リポジトリを再分析し、検証済みの概念図を台帳へ反映します。未関連付けの登録済みプロジェクトは、対象を一意に確認したうえで`.project-manager.json`も自動作成します。

概念図画面では空のJSONテンプレートを作成し、JSONの貼り付けまたはファイル読込後にpreview/commitできます。テンプレートの初期レイアウトは`human-stack-battle-20260715`準拠です。
