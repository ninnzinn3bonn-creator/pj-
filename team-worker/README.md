...
WorkerのURLを知っているだけでは共有データを取得できません。未ログインは401、チーム外ユーザーは403、変更APIの許可Origin不一致も403になります。ブラウザのログインセッションは7日ですが、ローカル台帳用の固定接続トークンは再発行またはメンバー解除まで有効です。固定トークンはメンバー・チーム単位で発行され、再発行すると旧トークンは直ちに無効になります。
